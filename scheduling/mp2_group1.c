...
#include "mp2_group1.h"
#include "mp2_given.h"

static struct mp2_task_struct process_list;
...
void _timer_callback(unsigned long pid)
{
   struct list_head* node;
	...
	list_for_each(node, &process_list.list) {
		task = list_entry(node, struct mp2_task_struct, list);
		if (task->pid == pid) {
			task->process_state = PROCESS_READY;
...
{ 
    ...
    //Admission Control
	if ((long long)(total_use + ((1000*computation)/period)) >= (long long)693) {
	
	   printk(KERN_INFO "Registration denied for process with pid %d",pid);
	   
	   return;
	}
   
   printk(KERN_INFO "REGISTER process with pid: %d |period: %d| comp: %d",pid,period,computation);
   
   task = (struct mp2_task_struct *)kmalloc(sizeof(struct mp2_task_struct),GFP_KERNEL);
   ...
   task->process_state = PROCESS_SLEEPING;
   
   //update total processing time and period
   total_use += (1000* task->computation)/task->period;
...
*
*/
struct mp2_task_struct* task_by_pid(int pid)
{
...
/*
* Performs a context switch initiated by the running process.
*
*/
void yield_process(unsigned int pid)
{
   ...
   if (task)
   {
       printk(KERN_INFO "YIELD task with pid %d", pid);     
       
       //compute next wake_up
       next_wakeup = get_jiffies_64() + msecs_to_jiffies(task->period);
       
       if (next_wakeup > (get_jiffies_64()- task->last_wakeup_time))
       {
          //printk(KERN_INFO "wakeup successfully scheduled for pid %d",task->pid);
          
		  //set the timer for the next interruption
          if(mod_timer(&task->wakeup_timer, next_wakeup))
          {
              return;
          }
          ...
          printk(KERN_INFO "Task with pid %d set to SLEEP", pid);
          
          set_task_state(task->linux_task,TASK_UNINTERRUPTIBLE);
       }
       else
       {
         printk(KERN_INFO "A wakeup cannot be scheduled for pid %d",task->pid);
       }
   }
}
...
void deregister_process(unsigned int pid)
{
   ...
   list_for_each_safe(pos, q, &process_list.list)
   {
      ...
      if (tmp->pid == pid)
      {
         ...
         total_use -= (1000*tmp->computation)/tmp->period;
		 ...
		 //delete the timer to prevent subsequent firing
...
int status_write(struct file *file, const char *buf, unsigned long count, void *data)
{
   ...
   switch(token[0])
   {
      case 'R':
         {
            ...
            //extract process information R, PID, PERIOD, COMPUTATION
            ...
            ret = kstrtoint((const char*)token, 10, &period);
            if (ret)
               period = 0; 
               
            token = strsep(&running,delimiters); 
               
            ret = kstrtoint((const char*)token, 10, &computation) ;
            if (ret)
               computation = 0; 
      ...
      case 'D':
         {
            //deregister the provided process
            //retrieve second token: pid
            token = strsep(&running,delimiters);
...
int dispatcher_thread_handler(void *data)
{
   ...
   struct mp2_task_struct *tmp,*new_task;
   struct list_head *pos;
